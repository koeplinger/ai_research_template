...
it: a short note in markdown under `paper/`, named `note_vN.md`, released
as the file itself with nothing built.
...
- **What is released with a version** is the file itself, since the
  deliverable is not built; nothing else. Scratch produced on the way is not committed. The prose is
...
3. The researcher directs the release: the `Released D Month YYYY` stamp
   ...
   written.
4. What releases with the version (here, the note file itself) is
   produced and committed by the researcher.
...
| File | What it is |
|---|---|
| [note_v1.md](note_v1.md) | the note, version 1: the roll's count against the note's figure; released 6 September 2026 after the review under `reviews/` |
