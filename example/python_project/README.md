...
`CHECK_METHODOLOGY.md`'s check form (its Slots table).
...
claim registered RULED_OUT, and its record says what the mutation does.

## Design rules

- **Runnable offline** after the dependencies are installed.
- **Deterministic**: fixed seeds wherever sampling is used.
- **The instrument of record is pinned.** Where the instrument of record is
...
- **Every mutation is bound.** Each name in a check's `Mutation:` line is
  a member of `shift-year` (every entry's year moved forward by one), `drop-month` (one month's entries removed), `duplicate-entry` (one entry repeated), `alter-figure` (a stated figure changed by one), bound in the program's `MUTATIONS` to a
...
|---|---|---|
| `{{CHECK_FORM}}`: an executable program with a printed verdict, or a written collation procedure against the source of record | what a check is in this project; this suite ships the program form (`CHECK_METHODOLOGY.md`) | see that file's Slots table |
| `{{VERIFICATION_TOOL}}`: the roll file as shipped under `source_documents/`, read with the Python standard library (3.11 or later) at its committed revision | the instrument of record, pinned in `requirements.txt` where it is software (`CHECK_METHODOLOGY.md` §3) | a computer-algebra library at an exact version; a statistics stack at exact versions with the data release named |
| `{{MUTATION_SET}}`: `shift-year` (every entry's year moved forward by one), `drop-month` (one month's entries removed), `duplicate-entry` (one entry repeated), `alter-figure` (a stated figure changed by one) | the alterations the probe applies (`CHECK_METHODOLOGY.md`) | see that file's Slots table |
