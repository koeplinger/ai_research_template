...
| [new_project.py](new_project.py) | The instantiation procedure (`GETTING_STARTED.md` §1 to §3): copies the template, leaves behind the build's own files and the pointers to them, strips the build lens, fills the sixteen slots and the values the tools read, renames any field the project's discipline has taken, adds the project's licenses beside the template's, writes a project front page, and runs the round check in the new tree. Asks for every value or reads them from an answers file; `--template` writes a blank one. `--selftest` instantiates into a scratch directory and requires the round check there to be clean. |
...
| [check_residue.py](check_residue.py) | The residue check: what a rebuild leaves in a live draft, listed for the reading that closes the round (`CHECK_METHODOLOGY.md` §7, which states the defect class in plain words): seams, dangling and orphaned references, phantoms, echoes, structure, and a live changelog's ledgers against the draft. The five coined names are this tool's, defined in `GLOSSARY.md`. No rulebook item; candidates for the reader, tags RESIDUE-1 to 7. Reads a typeset source when the pipeline is enabled. |
